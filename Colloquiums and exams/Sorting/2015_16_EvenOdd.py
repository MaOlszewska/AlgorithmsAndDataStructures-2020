...
def sort(T):
    ...
    for i in range(len(T)):
        if T[i] %  2 == 0:
            ...
            odd.append(T[i])  # nieparzyste są już posortowane

    even = quicksort(even, 0, len(even) - 1)   # sortuje liczby parzyste quicksortem

    # teraz wystarczy połączyć posortowane dwie tablice

    j = k = 0  # indeksy, przez które będe się poruszać w tych dwóch tablicach
